...
class MixCFN(nn.Module):
    ...
    def forward(self, x: Tensor, H: int, W: int) -> Tensor:
        def _inner_forward(x: Tensor) -> Tensor:
            x = self.fc1(x)
            B, N, C = x.shape
            x = self.conv(x.transpose(1, 2).contiguous().view(B, C, H, W))
            x = self.act(x)
            x = self.fc2(x.flatten(2).transpose(-1, -2).contiguous())
            return x
...
class Attention_axial_part(nn.Module):
    ...
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=6, attn_drop=0., proj_drop=0., qk_scale=None, position_bias=True):
        ...
        if idx == -1:
            ...
            H_sp, W_sp = self.split_size, self.resolution//4
        elif idx == 1:
            W_sp, H_sp = self.split_size, self.resolution//4
        else:
            print ("ERROR MODE", idx)
            exit(0)
        self.H_sp = H_sp
    ...
    def forward(self, qkv, H, W, mask=None):
        ...
        if self.resolution != H or self.resolution != W:
            if self.idx == -1:
                ...
                H_sp, W_sp = self.split_size, W//4
            elif self.idx == 1:
                W_sp, H_sp = self.split_size, H//4
            else:
                print ("ERROR MODE", self.idx)
                exit(0)
            self.H_sp = H_sp
...
class CATB_axial_cab(nn.Module):
    """ Axial Cross Aggregation Transformer Block.
    Args:
        dim (int): Number of input channels.
        reso (int): Input resolution.
    ...
    def __init__(self, dim, reso, num_heads,
        ...
        self.cab = nn.Sequential(
            nn.Conv2d(dim, dim//3, 3, 1, 1), nn.GELU(), nn.Conv2d(dim//3, dim, 3, 1, 1),
    ...
    def forward(self, x, x_size):
        ...
        attened_x = self.proj(attened_x)
        # cab
        x_cab = rearrange(img, "b (h w) c -> b c h w", h=H, w=W)
        x_cab = self.cab(x_cab)
        x_cab = rearrange(x_cab, "b c h w -> b (h w) c")

        x = x + self.drop_path(attened_x) + self.gamma*x_cab
        x = x + self.drop_path(self.mlp(self.norm2(x)))

        return x


class CATB_axial_part(nn.Module):
    ...
    def __init__(self, dim, reso, num_heads,
        ...
        self.attns = nn.ModuleList([
                Attention_axial_part(
                    dim//2, resolution=self.patches_resolution, idx = i,
                    split_size=split_size[0], num_heads=num_heads//2, dim_out=dim//2,
                    qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, position_bias=True)
                for i in range(self.branch_num)])
    ...
    def calculate_mask(self, H, W):
        ...
        h_slices_0 = (slice(0, -self.split_size[0]),
                    slice(-self.split_size[0], -self.shift_size[0]),
                    slice(-self.shift_size[0], None))
        w_slices_0 = (slice(0, -(W//self.split_size[1])),
                    slice(-(W//self.split_size[1]), -(W//self.shift_size[1])),
                    slice(-(W//self.shift_size[1]), None))

        h_slices_1 = (slice(0, -(H//self.split_size[1])),
                    slice(-(H//self.split_size[1]), -(H//self.shift_size[1])),
                    slice(-(H//self.shift_size[1]), None))
        w_slices_1 = (slice(0, -self.split_size[0]),
                    slice(-self.split_size[0], -self.shift_size[0]),
                    slice(-self.shift_size[0], None))
        cnt = 0
        ...
        # calculate mask for H-Shift
        img_mask_0 = img_mask_0.view(1, H // self.split_size[0], self.split_size[0], self.split_size[1], W // self.split_size[1], 1)
        img_mask_0 = img_mask_0.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, self.split_size[0], W // self.split_size[1], 1) # nW, sw[0], W//sw[1], 1
        mask_windows_0 = img_mask_0.view(-1, self.split_size[0] * (W // self.split_size[1]))
        attn_mask_0 = mask_windows_0.unsqueeze(1) - mask_windows_0.unsqueeze(2)
        attn_mask_0 = attn_mask_0.masked_fill(attn_mask_0 != 0, float(-100.0)).masked_fill(attn_mask_0 == 0, float(0.0))

        # calculate mask for V-Shift
        img_mask_1 = img_mask_1.view(1, self.split_size[1], H // self.split_size[1], W // self.split_size[0], self.split_size[0], 1)
        img_mask_1 = img_mask_1.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, H//self.split_size[1], self.split_size[0], 1) # nW, H//sw[1], sw[0], 1
        mask_windows_1 = img_mask_1.view(-1, (H // self.split_size[1]) * self.split_size[0])
        attn_mask_1 = mask_windows_1.unsqueeze(1) - mask_windows_1.unsqueeze(2)
        attn_mask_1 = attn_mask_1.masked_fill(attn_mask_1 != 0, float(-100.0)).masked_fill(attn_mask_1 == 0, float(0.0))
    ...
    def forward(self, x, x_size):
        ...
        if self.shift_size[0] > 0 or self.shift_size[1] > 0:
            qkv = qkv.view(3, B, H, W, C)
            # H-Shift
            qkv_0 = torch.roll(qkv[:,:,:,:,:C//2], shifts=(-self.shift_size[0], -(W//self.shift_size[1])), dims=(2, 3))
            qkv_0 = qkv_0.view(3, B, L, C//2)
            # V-Shift
            qkv_1 = torch.roll(qkv[:,:,:,:,C//2:], shifts=(-(H//self.shift_size[1]), -self.shift_size[0]), dims=(2, 3))
            qkv_1 = qkv_1.view(3, B, L, C//2)
            ...
            x1 = torch.roll(x1_shift, shifts=(self.shift_size[0], W//self.shift_size[1]), dims=(1, 2))
            x2 = torch.roll(x2_shift, shifts=(H//self.shift_size[1], self.shift_size[0]), dims=(1, 2))
            x1 = x1.view(B, L, C//2).contiguous()
            x2 = x2.view(B, L, C//2).contiguous()
            # Concat
            attened_x = torch.cat([x1,x2], dim=2)
...
class CATB_axial_mix(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        # mix
        lcm = self.dwconv3x3(v)
        channel_interaction = self.channel_interaction(lcm).permute(0, 2, 3, 1).contiguous().view(B, 1, C)
        attention_spatial = attened_x.transpose(-2,-1).contiguous().view(B, C, H, W)
        spatial_interaction = self.spatial_interaction(attention_spatial)

        attened_x = attened_x * torch.sigmoid(channel_interaction)
        lcm = torch.sigmoid(spatial_interaction) * lcm
        lcm = lcm.permute(0, 2, 3, 1).contiguous().view(B, L, C)
...
class CATB_axial_mix_dwmlp(nn.Module):
    ...
    def __init__(self, dim, reso, num_heads,
        ...
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.mlp = MixCFN(in_features=dim, hidden_features=mlp_hidden_dim, out_features=dim, act_layer=act_layer)
        self.norm2 = norm_layer(dim)
...
class ResidualGroup(nn.Module):
    ...
    def __init__(   self,
        ...
        elif block_name == 'CATB_axial_cab':
            self.blocks = nn.ModuleList([
            CATB_axial_cab(
                dim=dim,
                num_heads=num_heads,
        ...
        elif block_name == 'CATB_axial_part':
            self.blocks = nn.ModuleList([
            CATB_axial_part(
                dim=dim,
                num_heads=num_heads,
        ...
        elif block_name == 'CATB_axial_mix':
            self.blocks = nn.ModuleList([
            CATB_axial_mix(
                dim=dim,
                num_heads=num_heads,
        ...
        elif block_name == 'CATB_axial_mix_dwmlp':
            self.blocks = nn.ModuleList([
            CATB_axial_mix_dwmlp(
                dim=dim,
                num_heads=num_heads,
                reso=reso,
                mlp_ratio=mlp_ratio,
...
class CAT(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # pad
        patch_size1 = max(self.split_size_0)
        patch_size2 = max(self.split_size_1)
        ...
        x = torch.cat([x, torch.flip(x, [2])], 2)[:, :, :h+mod_pad_h, :]
        x = torch.cat([x, torch.flip(x, [3])], 3)[:, :, :, :w+mod_pad_w]
        ...
        _, _, h, w = x.size()
        x = x[:, :, 0:h - mod_pad_h * self.upscale, 0:w - mod_pad_w * self.upscale]
        return x
